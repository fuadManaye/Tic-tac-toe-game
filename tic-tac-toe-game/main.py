...
def checkWin():
    ...
    for i in range(3):
        if grd[0][i]==grd[1][i]==grd[2][i]!=0:
            for j in range(3):
                curr[j][i].config(bg="green",fg="black")
            gamestatus=False
    for i in range(3):
        if grd[i][0]==grd[i][1]==grd[i][2]!=0:
            for j in range(3):
                curr[i][j].config(bg="green",fg="black")
            gamestatus=False
    if grd[0][2]==grd[1][1]==grd[2][0]!=0:
            curr[0][2].config(bg="green",fg="black")
            curr[1][1].config(bg="green",fg="black")
            curr[2][0].config(bg="green",fg="black")
            gamestatus=False
    if grd[0][0]==grd[1][1]==grd[2][2]!=0:
            for j in range(3):
                curr[j][j].config(bg="green",fg="black")
            gamestatus=False
